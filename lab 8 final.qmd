...
fish %>%
  #summarise all the nas
  summarise(across(everything(), ~ sum(is.na(.)))) %>%
  #outputs variable with nas and count of nas
...
#filter by nas
na_counts <- map_int(fish, ~ sum(is.na(.)))

#tidy tibble showing the nas
tibble(variable = names(na_counts), na_count = na_counts) %>%
  filter(na_count > 0)
...
Using min(length) for the weight variable. Need to use weight!
...
#create function of x - min(x) without NAs / max(x) w/o NAs
rescale_01 <- function(x) {
  (x - min(x, na.rm = TRUE)) /
	(max(x, na.rm = TRUE) - min(x, na.rm = TRUE))
}
#see if it works and applies properly
fish %>%
  mutate(across(c(length, weight), rescale_01))
...
#edited to created plot_1 name
plot_1 <- fish |>  
  ggplot(aes(x = length)) +
  geom_histogram(binwidth = 45, fill = "darkblue", color = "white") +
  labs(x = "Original Values of Fish Length (mm)") +
  scale_y_continuous(limits = c(0,4000))

#adding rescaled length
fish <- fish %>%
  mutate(length_rescaled = rescale_01(length))
#diff colors for easy comparison. same limits to show different counts at respective points
 plot_2 <- ggplot(fish, aes(x = length_rescaled)) +
  geom_histogram(bins = 30, fill = "darkgreen", color = "white") +
  ggtitle("Rescaled Length Distribution (0-1)") +
   scale_y_continuous(limits = c(0,4000))
 #side by side plots
 plot_1 + plot_2
```
